...
export default function PremiumReport({ data }) {
  ...
  // Extract data from calculatedMetrics (real converted values)
...
`;
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`Product_Report_${productName.replace(/ /g, '_')}.pdf`);
        ...
        <div className="relative z-10 space-y-8">
          ...
          <div className="cyber-card rounded-2xl p-6 border-l-4 flex flex-wrap items-center justify-between gap-6" style={{ borderLeftColor: actionColor }}>
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="text-center"><p className="text-[9px] text-gray-500 font-mono uppercase">Risk</p><p className="text-xl font-bold text-yellow-400">{data.riskMeter || 'Medium'}</p></div>
          ...
          <div className="cyber-card rounded-2xl p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div><p className="text-[10px] text-gray-500 font-mono">👤 Demographic</p><p className="text-sm text-white font-medium">{playbook.targetDemographic || 'Young adults 18-35'}</p>
                <div className="mt-4"><p className="text-[10px] text-gray-500 font-mono">😤 Top Pain Points</p><ul className="list-disc list-inside text-sm text-gray-300 mt-1">{painPoints.length > 0 ? painPoints.slice(0, 3).map((p, i) => <li key={i}>{p}</li>) : <li>No pain points data available</li>}</ul></div>
              </div>
              <div><p className="text-[10px] text-gray-500 font-mono">📢 Ad Headlines</p><ul className="list-disc list-inside text-sm text-gray-300 mt-1">{playbook.adHeadlines?.length > 0 ? playbook.adHeadlines.slice(0, 3).map((h, i) => <li key={i}>{h}</li>) : <li>No ad headlines available</li>}</ul>
          ...
          <div className="cyber-card rounded-2xl p-6">
            ...
            <div className="flex flex-wrap items-center gap-6">
              <div className="flex-1"><p className="text-[10px] text-gray-500 font-mono">Sentiment Breakdown</p>
                <div className="flex h-3 rounded-full overflow-hidden mt-1"><div className="bg-green-500" style={{ width: `${sentiment.positive || 0}%` }} /><div className="bg-yellow-500" style={{ width: `${sentiment.neutral || 0}%` }} /><div className="bg-red-500" style={{ width: `${sentiment.negative || 0}%` }} /></div>
                <div className="flex justify-between text-[8px] text-gray-500 mt-0.5"><span>👍 {sentiment.positive || 0}%</span><span>😐 {sentiment.neutral || 0}%</span><span>👎 {sentiment.negative || 0}%</span></div>
              </div>
          ...
          <div className="cyber-card rounded-2xl p-6">
            ...
            <ul className="space-y-2">{recommendations.map((rec, idx) => <li key={idx} className="flex items-start gap-2 bg-[#0F172A] p-3 rounded-xl border border-[#2dd4bf]/5"><span className="text-[#2dd4bf] font-bold text-sm">{idx+1}.</span><span className="text-sm text-gray-200">{rec}</span></li>)}</ul>
          </div>
          ...
          <div className="cyber-card rounded-2xl p-6 border-l-4 border-l-[#2dd4bf]">
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3 bg-[#0F172A] p-3 rounded-xl border border-[#2dd4bf]/5"><span className="text-sm font-bold text-green-400">1.</span><p className="text-sm text-gray-200">{actionScore >= 70 ? 'YES. The market shows strong potential with clear gaps.' : 'CONSIDER. The market has moderate potential but requires differentiation.'}</p></div>
              <div className="flex items-start gap-3 bg-[#0F172A] p-3 rounded-xl border border-[#2dd4bf]/5"><span className="text-sm font-bold text-yellow-400">2.</span><p className="text-sm text-gray-200">Biggest Risk: High competition and price sensitivity. Mitigate by focusing on quality and unique value proposition.</p></div>
              <div className="flex items-start gap-3 bg-[#0F172A] p-3 rounded-xl border border-[#2dd4bf]/5"><span className="text-sm font-bold text-[#2dd4bf]">3.</span><p className="text-sm text-gray-200">First 3 Steps THIS WEEK: 1) Order samples from 3 suppliers. 2) Create a landing page with real reviews. 3) Start micro-influencer outreach.</p></div>
            </div>
          </div>

          {/* FOOTER */}
          <div className="flex items-center justify-between text-[9px] text-gray-600 font-mono border-t border-[#2dd4bf]/10 pt-4"><span>PROFITFORGE Pro v6.0 · Premium Product Research</span><span>Real Data · Actionable · Ready to Launch</span><span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-[#34d399] animate-pulse" /> Live</span></div>
        </div>
      </motion.div>
    );
  }

  // Fallback for SEO (should not happen if SEOReport component is used)
