...
const MetricCard = ({ label, value, max = 10, color = '#2dd4bf' }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-gradient-to-br from-[#0F172A] to-[#1E293B] p-5 rounded-xl border border-[#2dd4bf]/10 shadow-lg hover:shadow-[#2dd4bf]/10 transition-all"
    >
...
export default function PremiumReport({ data }) {
  ...
  // --- Price Spread Chart Data (Rounded Bars) ---
  const chartData =
...
*Report generated by PROFITFORGE v5.0 • Real Data*
`;
    return md;
  };
  ...
  return (
    <motion.div
      ...
      className="max-w-6xl mx-auto mt-10 space-y-6 p-2 bg-[#080B12]"
    >
      {/* ========== HEADER ========== */}
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[#2dd4bf]/10 pb-4">
        <div className="flex items-center gap-3">
          <h2 className="text-2xl font-bold text-white tracking-tight">{data.productName}</h2>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button onClick={copyMarkdown} className="text-xs bg-[#0F172A] hover:bg-[#1E293B] px-3 py-1.5 rounded-lg border border-[#2dd4bf]/20 flex items-center gap-1 text-gray-300 font-mono transition">
      ...
      <div className="cyber-card rounded-2xl p-6 border-l-4 flex flex-wrap items-center justify-between gap-6" style={{ borderLeftColor: actionColor }}>
        <div className="flex items-center gap-6">
          ...
          <div>
            <p className="text-xs text-gray-400 font-mono">ACTION SCORE</p>
            <p className="text-xl font-bold" style={{ color: actionColor }}>{actionLabel}</p>
            <p className="text-sm text-gray-300 max-w-md truncate">{data.executiveSummary || 'Analysis complete.'}</p>
          </div>
        </div>
      ...
      {/* ========== ROW 2: COMPETITOR INTEL + PRICE SPREAD ========== */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="cyber-card rounded-2xl p-6">
          ...
          <div className="space-y-4">
            <div>
              <p className="text-xs text-gray-400 font-mono">Product</p>
              ...
              <p className="text-xs text-gray-400 font-mono mt-1">{comp.dominantBrands?.join(' • ') || 'Top Brands'}</p>
            </div>
            <div className="grid grid-cols-2 gap-4 bg-[#0F172A] p-4 rounded-xl border border-[#2dd4bf]/5">
              <div>
                <p className="text-xs text-gray-400 font-mono flex items-center gap-1"><Truck size={12} /> Est. Shipping</p>
              ...
              <div>
                <p className="text-xs text-gray-400 font-mono">Competitors</p>
            ...
            {/* ✅ MARKET GAP - Now Visible */}
            <div className="p-3 bg-[#0F172A] rounded-xl border-l-2 border-[#a78bfa]">
              <p className="text-xs text-gray-400 font-mono flex items-center gap-1"><Target size={12} /> Market Gap</p>
              <p className="text-sm text-gray-300">{data.marketGap?.description || 'Stable market with opportunities.'}</p>
            </div>
          </div>
        </div>

        {/* RIGHT: Price Spread with ROUNDED Bars */}
        <div className="cyber-card rounded-2xl p-6">
          <div className="flex items-center gap-2 text-xs text-[#2dd4bf] font-mono uppercase tracking-wider mb-4 border-b border-[#2dd4bf]/10 pb-2">
            <BarChart3 size={14} /> PRICE SPREAD
          </div>
          <div className="h-56 w-full bg-[#0F172A] p-2 rounded-xl border border-[#2dd4bf]/5">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} layout="vertical">
                ...
                />
                <Bar dataKey="price" fill="#2dd4bf" radius={[0, 8, 8, 0]} barSize={16}>
          ...
          {/* ✅ SENTIMENT ANALYSIS - Now Visible */}
          <div className="mt-4">
            <p className="text-xs text-gray-400 font-mono flex items-center gap-1"><MessageCircle size={12} /> Sentiment</p>
            ...
            {/* ✅ TOP PAIN POINTS - Visible */}
            {sentiment.topPainPoints?.length > 0 && (
              <div className="mt-2 flex flex-wrap gap-1">
                {sentiment.topPainPoints.map((p, i) => (
                  <span key={i} className="text-[10px] bg-red-500/10 text-red-400 px-2 py-0.5 rounded-full border border-red-500/20">{p}</span>
                ))}
              </div>
            )}
          </div>
      ...
      {/* ========== ROW 3: V5.0 ADV DATA + PROFIT/ROI ========== */}
        ...
        <div className="cyber-card rounded-2xl p-6">
          ...
          <div className="space-y-4">
            <div className="bg-gradient-to-br from-[#0F172A] to-[#1E293B] p-4 rounded-xl border border-[#2dd4bf]/10">
              <p className="text-xs text-gray-400 font-mono">Profit (Real)</p>
              <p className={`text-3xl font-bold ${calc.profit > 0 ? 'text-[#34d399]' : 'text-red-400'}`}>
                {symbol}{formatPrice(calc.profit)}
              </p>
            </div>
            <div className="bg-gradient-to-br from-[#0F172A] to-[#1E293B] p-4 rounded-xl border border-[#2dd4bf]/10">
              <p className="text-xs text-gray-400 font-mono">ROI (Real)</p>
      ...
      <div className="cyber-card rounded-2xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* ✅ TARGET DEMOGRAPHIC - Fixed */}
          <div className="bg-gradient-to-br from-[#0F172A] to-[#1E293B] p-4 rounded-xl border border-[#2dd4bf]/5">
            <h4 className="text-xs text-gray-400 font-mono flex items-center gap-1"><Users size={12} /> Target Demographic</h4>
            <p className="text-sm text-white mt-1">{playbook.targetDemographic || 'General audience'}</p>
          </div>

          <div className="bg-gradient-to-br from-[#0F172A] to-[#1E293B] p-4 rounded-xl border border-[#2dd4bf]/5">
            <h4 className="text-xs text-gray-400 font-mono flex items-center gap-1"><Sparkles size={12} /> Ad Headlines</h4>
          ...
          <div className="bg-gradient-to-br from-[#0F172A] to-[#1E293B] p-4 rounded-xl border border-[#2dd4bf]/5">
            <h4 className="text-xs text-gray-400 font-mono flex items-center gap-1"><Target size={12} /> Interests & Communities</h4>
        ...
        {/* ✅ ADVANCED INSIGHTS - Fixed with proper labels */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4 pt-4 border-t border-[#2dd4bf]/10">
          <div>
            <p className="text-[10px] text-gray-400 font-mono flex items-center gap-1"><Truck size={12} /> Supplier Suggestion</p>
            <p className="text-xs text-white">{adv.supplierSuggestion || 'AliExpress'}</p>
          </div>
          <div>
            <p className="text-[10px] text-gray-400 font-mono flex items-center gap-1"><Calendar size={12} /> Seasonality</p>
            <p className="text-xs text-white">{adv.seasonality || 'Year-round'}</p>
          </div>
          <div>
            <p className="text-[10px] text-gray-400 font-mono flex items-center gap-1"><Hash size={12} /> Top Keywords</p>
