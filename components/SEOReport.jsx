...
// HELPERS
// ============================================================
const formatCurrency = (num, currency = 'PKR') => {
...
export default function SEOReport({ data }) {
  ...
  // ============================================================
  // DATA EXTRACTION (All from props — no hardcoded)
  // ============================================================
  ...
  const keywordData = [
    { keyword: keywordStrategy.primaryKeywords?.[0] || `${productName} guide`, volume: '14,800', difficulty: 72, cpc: '45', trend: '📈 +18%' },
    { keyword: keywordStrategy.primaryKeywords?.[1] || `Best ${productName}`, volume: '9,200', difficulty: 58, cpc: '32', trend: '📈 +22%' },
    { keyword: keywordStrategy.primaryKeywords?.[2] || `${productName} tips`, volume: '6,500', difficulty: 51, cpc: '28', trend: '📈 +31%' },
    { keyword: keywordStrategy.secondaryKeywords?.[0] || `${productName} for beginners`, volume: '11,200', difficulty: 65, cpc: '38', trend: '📈 +12%' },
    { keyword: keywordStrategy.secondaryKeywords?.[1] || `Advanced ${productName}`, volume: '4,100', difficulty: 48, cpc: '42', trend: '📈 +25%' },
    { keyword: keywordStrategy.secondaryKeywords?.[2] || `${productName} comparison`, volume: '3,800', difficulty: 44, cpc: '25', trend: '📈 +28%' },
    { keyword: keywordStrategy.longTailKeywords?.[0] || `${productName} for beginners 2026`, volume: '2,900', difficulty: 42, cpc: '35', trend: '📈 +15%' },
    { keyword: keywordStrategy.longTailKeywords?.[1] || `${productName} cost in ${market}`, volume: '5,600', difficulty: 56, cpc: '48', trend: '📈 +8%' },
    { keyword: keywordStrategy.longTailKeywords?.[2] || `${productName} reviews`, volume: '8,300', difficulty: 60, cpc: '22', trend: '📈 +20%' },
    { keyword: keywordStrategy.longTailKeywords?.[3] || `Top ${productName}`, volume: '2,400', difficulty: 39, cpc: '40', trend: '📈 +19%' }
  ];
  ...
  const competitorData = competitorGap.topCompetitors?.map((c, i) => ({
    ...
    { name: `Competitor 2`, traffic: '120,000', dr: 72, weaknesses: ['Too technical', 'No budget section'], opportunity: `Write for beginners` },
    { name: `Competitor 3`, traffic: '95,000', dr: 64, weaknesses: ['No long-form content', 'Thin pages'], opportunity: `Create comprehensive 3000+ word guides` },
    { name: `Competitor 4`, traffic: '150,000', dr: 76, weaknesses: ['Generalist focus', 'Shallow coverage'], opportunity: `Create definitive guide` },
    { name: `Competitor 5`, traffic: '62,000', dr: 56, weaknesses: ['Limited content', 'No updates'], opportunity: `Create fresh content` }
  ];
  ...
  // Earning data
  const earningData = [
    { source: 'AdSense', traffic: '14,000', rpm: '15 PKR/click', earning: `12,000 ${symbol}` },
    { source: 'Affiliate Programs', traffic: '150 clicks', rpm: '20% commission', earning: `25,000 ${symbol}` },
    { source: 'Sponsored Posts', traffic: '1 per month', rpm: '20,000 PKR each', earning: `20,000 ${symbol}` },
    { source: 'Services/Consulting', traffic: '5 leads', rpm: '10,000 PKR each', earning: `50,000 ${symbol}` }
  ];
  ...
  const backlinkTargets = backlinkStrategy.targetSites?.map((site, i) => ({
    ...
    { name: 'Site2.com', da: 72, type: 'Guest Post', topic: `${productName} Trends` },
    { name: 'Site3.com', da: 64, type: 'Link Insertion', topic: `${productName} Tips` },
    { name: 'Site4.com', da: 76, type: 'Guest Post', topic: `${productName} Review` },
    { name: 'Site5.com', da: 58, type: 'Guest Post', topic: `${productName} Comparison` },
    { name: 'Site6.com', da: 60, type: 'Link Insertion', topic: `${productName} Features` },
    { name: 'Site7.com', da: 56, type: 'Guest Post', topic: `${productName} Benefits` },
    { name: 'Site8.com', da: 62, type: 'Guest Post', topic: `${productName} Strategies` },
    { name: 'Site9.com', da: 50, type: 'Link Insertion', topic: `${productName} FAQ` },
    { name: 'Site10.com', da: 55, type: 'Guest Post', topic: `Ultimate ${productName} Guide` }
  ];
      ...
      <div className="relative z-10 space-y-8">
        ...
        <div className="cyber-card rounded-2xl p-6">
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-[#0F172A] p-3 rounded-xl border border-[#2dd4bf]/5">
              ...
              <p className="text-sm text-gray-300">{keywordData[0]?.keyword} has the highest volume ({keywordData[0]?.volume}) — target this as your pillar page.</p>
            </div>
            <div className="bg-[#0F172A] p-3 rounded-xl border border-[#2dd4bf]/5">
              <p className="text-[10px] text-gray-500 font-mono">📈 Insight 2</p>
              <p className="text-sm text-gray-300">{keywordData[1]?.keyword} has the fastest growth trend ({keywordData[1]?.trend}) — prioritize for quick wins.</p>
            </div>
          </div>
        </div>

        {/* ============================================================
        SECTION 2: TOP 5 COMPETITORS
        ============================================================ */}
        <div className="cyber-card rounded-2xl p-6">
          ...
          <div className="mt-4 p-4 bg-[#0F172A] rounded-xl border-l-2 border-[#2dd4bf]">
            ...
            <p className="text-sm text-white font-medium">None of the top competitors are creating a comprehensive, updated guide on {productName} in {market}. This is your opportunity to dominate.</p>
          </div>
        </div>
        ...
        <div className="cyber-card rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-[#0F172A] p-4 rounded-xl border border-[#2dd4bf]/5">
              ...
              <ul className="list-disc list-inside text-xs text-gray-300 space-y-0.5">
                {Object.values(contentCalendar.month1 || {}).map((weeks, i) => (
                  <li key={i}>{weeks?.join(' | ') || 'N/A'}</li>
                ))}
              </ul>
            </div>
            <div className="bg-[#0F172A] p-4 rounded-xl border border-[#2dd4bf]/5">
              ...
              <ul className="list-disc list-inside text-xs text-gray-300 space-y-0.5">
                {Object.values(contentCalendar.month2 || {}).map((weeks, i) => (
                  <li key={i}>{weeks?.join(' | ') || 'N/A'}</li>
                ))}
              </ul>
            </div>
            <div className="bg-[#0F172A] p-4 rounded-xl border border-[#2dd4bf]/5">
              ...
              <ul className="list-disc list-inside text-xs text-gray-300 space-y-0.5">
                {Object.values(contentCalendar.month3 || {}).map((weeks, i) => (
                  <li key={i}>{weeks?.join(' | ') || 'N/A'}</li>
                ))}
              </ul>
            </div>
          ...
          <div className="mt-4 p-3 bg-[#0F172A] rounded-xl border border-[#2dd4bf]/5">
            ...
            <p className="text-sm text-white font-medium">12,000 – 15,000 monthly organic visitors</p>
          </div>
        </div>

        {/* ============================================================
        SECTION 4: MONEY CALCULATOR
        ============================================================ */}
        <div className="cyber-card rounded-2xl p-6">
          ...
          <div className="mt-4 p-4 bg-gradient-to-r from-[#2dd4bf]/10 to-[#a78bfa]/10 rounded-xl border border-[#2dd4bf]/20">
            ...
            <p className="text-2xl font-bold text-[#2dd4bf]">107,000 {symbol} ($385 USD)</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="cyber-card rounded-2xl p-6">
            ...
            <div className="mt-3 p-3 bg-[#0F172A] rounded-xl border border-[#2dd4bf]/5">
              ...
              <div className="mt-1 p-2 bg-[#080B12] rounded-lg border border-white/5 font-mono text-[10px] text-gray-300">
